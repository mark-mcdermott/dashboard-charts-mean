...
export class HeartRateComponent implements AfterContentInit {
        ...
        this.chartService.getChartData(this.chartName).pipe(map((data: any) => data))
          .subscribe((res) => {
            ...
            this.dateArr = this.chartDataTable.map(bp => {
              let dateObj = new Date(bp['date'] + ' ' + bp['start']);
              return dateObj
            });

            // put the data in chart-ready arrays
            this.minArr = this.chartDataTable.map( bp => bp['min']);
            this.maxArr = this.chartDataTable.map(bp => bp['max']);
            this.averageArr = this.chartDataTable.map(bp => bp['average']);
